...
export default function FlashcardDeck() {
  ...
  return (
    <div className="h-full flex flex-col min-h-screen relative w-full" style={{ fontFamily: "var(--font-family)", background: "var(--background)" }}>
      <AnimatePresence mode="wait">
        {isLoading ? (
          ...
          <motion.div
            ...
            className="h-full min-h-screen flex items-center justify-center text-center p-4 w-full"
          >
          ...
          <motion.div
            ...
            exit={{ opacity: 0 }}
            transition={{ duration: 0.2 }}
            className="h-full min-h-screen flex items-center justify-center w-full"
          >
            <div className="text-center max-w-md w-full px-4">
              <div style={{ fontSize: 80, marginBottom: 16 }}>🎉</div>
              <h2 style={{ fontSize: 28, fontWeight: 900, color: "var(--foreground)", marginBottom: 8 }}>{t("sessionComplete")}</h2>
              <p style={{ fontSize: 15, color: "var(--muted-foreground)", marginBottom: 32 }}>
                {t("reviewedInsight", { count: Object.keys(gradedCards).length.toString() })}
              </p>
              
              <div className="flex gap-4 mb-8">
                <div className="flex-1 rounded-2xl p-4 text-center border-2 border-red-100 bg-red-50">
                  <div style={{ fontWeight: 800, fontSize: 24, color: "#FF4B4B" }}>{Object.values(gradedCards).filter(s => s === 0).length}</div>
                ...
                <div className="flex-1 rounded-2xl p-4 text-center border-2 border-green-100 bg-green-50">
                  <div style={{ fontWeight: 800, fontSize: 24, color: "#22c55e" }}>{Object.values(gradedCards).filter(s => s === 3).length}</div>
              ...
              >
                ...
                {dueCards.filter(card => gradedCards[card.id] === 0).length > 0 ? t("reviewForgottenBtn", { count: dueCards.filter(card => gradedCards[card.id] === 0).length.toString() }) : t("startNewSessionBtn")}
              </button>
            </div>
          </motion.div>
        ) : (
          <motion.div
            ...
            exit={{ opacity: 0 }}
            transition={{ duration: 0.2 }}
            className="h-full flex flex-col w-full text-left"
          >
            {/* Header */}
            <div className="px-8 py-6 border-b" style={{ borderColor: "var(--border)", background: "var(--card)" }}>
              ...
              <div className="rounded-full overflow-hidden" style={{ height: 8, background: "var(--muted)" }}>
                <motion.div
                  initial={{ width: 0 }}
                  animate={{ width: `${(graded / totalDue) * 100}%` }}
              ...
              <div className="flex-1 flex flex-col items-center justify-center gap-6 px-8 py-6 overflow-y-auto">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <motion.button
                    ...
                    whileHover={currentIndex !== totalDue - 1 ? { scale: 1.05 } : {}}
                    whileTap={currentIndex !== totalDue - 1 ? { scale: 0.95 } : {}}
                    className="w-9 h-9 rounded-full flex items-center justify-center cursor-pointer border-none outline-none"
                ...
                >
                  ...
                  >
                    {/* Front */}
                    <div
                      className="rounded-3xl p-8 flex flex-col items-center justify-center text-center absolute inset-0"
                      style={{
                    ...
                    >
                      ...
                      <button
                        className="flex items-center gap-2 px-4 py-2 rounded-full cursor-pointer hover:bg-sky-100 dark:hover:bg-sky-950/20 transition-colors border-none outline-none"
                      ...
                      <div className="flex gap-4 w-full">
                        {GRADE_OPTIONS.map(g => (
                          <motion.button
                            ...
                            whileTap={{ scale: 0.98 }}
                            className="flex-1 flex flex-col items-center justify-center py-4 rounded-2xl cursor-pointer transition-all border-none outline-none shadow-sm"
                            style={{
